...
pub(crate) fn parse_scope_directive(input: &str) -> ParsedDirective {
    ...
    for token in input.split_whitespace() {
        ...
        if pending_rows_value {
            result_rows = normalized.parse::<usize>().ok();
            pending_rows_value = false;
            continue;
        }
        ...
        if normalized == "/fullheight" {
            toggle_fullheight = true;
            continue;
        }
        ...
        if let Some(value) = normalized.strip_prefix("/rows:") {
            result_rows_directive = true;
            result_rows = value.parse::<usize>().ok();
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn parses_drive_scope_and_query() {
        let parsed = parse_scope_directive("/d: readme");
    ...
    fn parses_result_rows() {
        ...
        assert_eq!(parsed.result_rows, Some(40));

        let parsed = parse_scope_directive("/rows:32");
        assert!(parsed.result_rows_directive);
